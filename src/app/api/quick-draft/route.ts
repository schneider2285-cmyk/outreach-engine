  ...
  try {
    ...
    if (input.verified_triggers?.length)
      artifacts.push({
        ...
        content: { events: input.verified_triggers.map(t => ({ event: t.text, date_approx: 'recent', relevance: 'high' })) }
      });

    if (input.role_pain_hypothesis)
      artifacts.push({
        artifact_type: 'pain_points',
        content: { pains: [{ topic: input.role_pain_hypothesis, description: input.role_pain_hypothesis, evidence_index: 1 }] }
      });
    ...
    if (input.your_proof_points?.length)
      artifacts.push({
        ...
        content: { hooks: input.your_proof_points.map(p => ({ type: 'proof_point', detail: p.text, angle: 'credibility' })) }
      });
    ...
    const { drafts: draftOutputs, usage: draftUsage } = await generateDrafts(
      input.prospect_name, input.prospect_title, input.company, undefined,
      artifacts, draftChannel as 'email' | 'linkedin', 3
    );
    ...
    for (const draft of draftOutputs) {
      ...
      if (!lint.passed) {
        ...
        const { draft: rewritten, usage: rwU } = await rewriteDraft(
          draft, null, input.prospect_name, input.prospect_title, input.company, artifacts, fixInstr
        );
    ...
    for (const draft of lintedDrafts) {
      ...
      const { result: gate, usage: gU } = await empathyGate(
        draft, input.prospect_name, input.prospect_title, input.company,
      ...
      if (!gate.passes && gate.rewrite_actions?.length > 0) {
        ...
        finalDrafts.push({
          draft: rewritten, gate: reGate, was_rewritten: true, voice_lint: lintResult,
          original_gate: {
            passes: gate.passes, weakest_gate: gate.weakest_gate,
            gate_1: gate.gate_1_open?.verdict, gate_2: gate.gate_2_read?.verdict, gate_3: gate.gate_3_respond?.verdict
          }
        });
      } else {
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        passed: finalDrafts.filter((d: any) => d.gate.passes).length,
        failed: finalDrafts.filter((d: any) => !d.gate.passes).length,
        rewritten: finalDrafts.filter((d: any) => d.was_rewritten).length,
      },
